...
export default function AuditPage() {
  ...
  const columns = [
    ...
    { key: 'event_type', header: 'Event', render: (a: any) => <span className="px-2 py-0.5 rounded bg-gray-100 text-xs font-medium text-gray-700">{a.event_type}</span> },
    { key: 'actor', header: 'Actor', render: (a: any) => (
      <div><p className="text-sm">{a.actor_user_id?.substring(0, 8) || 'System'}...</p><p className="text-xs text-gray-500 capitalize">{a.actor_role || 'N/A'}</p></div>
    )},
    { key: 'target', header: 'Target', render: (a: any) => (
      <div><p className="text-sm capitalize">{a.target_entity_type || 'N/A'}</p><p className="text-xs text-gray-500 font-mono">{a.target_entity_id?.substring(0, 8) || ''}...</p></div>
    )},
