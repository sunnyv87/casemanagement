...
export default function UsersPage() {
  ...
  const columns = [
    { key: 'name', header: 'Name', render: (u: any) => <span className="font-medium text-gray-900">{u.first_name} {u.last_name}</span> },
    ...
    { key: 'last_login', header: 'Last Login', render: (u: any) => u.last_login_at ? <span className="text-xs text-gray-500">{formatDate(u.last_login_at)}</span> : <span className="text-gray-400 text-xs">Never</span> },
  ];
          ...
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-lg p-6">
            ...
            <form onSubmit={e => { e.preventDefault(); createMutation.mutate(newUser); }} className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                <div><label className="block text-sm font-medium text-gray-700 mb-1">First Name *</label><input type="text" className="input-field" required value={newUser.first_name} onChange={e => setNewUser(u => ({ ...u, first_name: e.target.value }))} /></div>
                <div><label className="block text-sm font-medium text-gray-700 mb-1">Last Name *</label><input type="text" className="input-field" required value={newUser.last_name} onChange={e => setNewUser(u => ({ ...u, last_name: e.target.value }))} /></div>
              </div>
              <div><label className="block text-sm font-medium text-gray-700 mb-1">Email *</label><input type="email" className="input-field" required value={newUser.email} onChange={e => setNewUser(u => ({ ...u, email: e.target.value }))} /></div>
              <div><label className="block text-sm font-medium text-gray-700 mb-1">Password *</label><input type="password" className="input-field" required minLength={8} value={newUser.password} onChange={e => setNewUser(u => ({ ...u, password: e.target.value }))} /></div>
