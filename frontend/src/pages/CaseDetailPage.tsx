...
export default function CaseDetailPage() {
  ...
  return (
    <div className="space-y-6">
      <button onClick={() => navigate('/cases')} className="flex items-center gap-2 text-gray-600 hover:text-gray-900">
      ...
      <div className="flex items-start justify-between flex-wrap gap-4">
        <div>
          <div className="flex items-center gap-3">
            <h1 className="text-2xl font-bold text-gray-900">{c.case_number}</h1>
          ...
          <h2 className="text-lg text-gray-700 mt-1">{c.title}</h2>
          <p className="text-sm text-gray-500 mt-1">{c.customer_name} | Created {formatDate(c.created_at)}</p>
        </div>
      </div>
          ...
          {c.alerts?.length > 0 && (
            <div className="card">
              ...
              <div className="space-y-2">
                {c.alerts.map((a: any) => (
                  <div key={a.alert_id} onClick={() => navigate(`/alerts/${a.alert_id}`)} className="flex items-center justify-between p-2 rounded-lg border border-gray-200 hover:bg-gray-50 cursor-pointer">
                    <div className="flex items-center gap-2">
                      <SeverityBadge severity={a.severity} />
                      <span className="text-sm font-medium">{a.alert_name}</span>
                    </div>
                    <span className="text-xs text-gray-500">{a.source_platform}</span>
                  </div>
                ))}
              </div>
            </div>
          ...
          <div className="card">
            ...
            <div className="space-y-4">
              {c.comments?.map((comment: any) => (
                <div key={comment.id} className={`p-4 rounded-lg border ${comment.is_internal ? 'border-yellow-200 bg-yellow-50' : comment.comment_type === 'customer_query' ? 'border-blue-200 bg-blue-50' : 'border-gray-200'}`}>
                  <div className="flex items-center justify-between mb-2">
                    <div className="flex items-center gap-2">
                      <span className="text-sm font-medium text-gray-900">{comment.first_name} {comment.last_name}</span>
                      <span className="text-xs text-gray-500 capitalize">({comment.role})</span>
                      {comment.is_internal && <span className="text-xs bg-yellow-200 text-yellow-800 px-1.5 py-0.5 rounded">Internal</span>}
                      {comment.comment_type !== 'comment' && <span className="text-xs bg-gray-200 text-gray-700 px-1.5 py-0.5 rounded">{comment.comment_type.replace('_', ' ')}</span>}
                    </div>
              ...
              {(!c.comments || c.comments.length === 0) && <p className="text-gray-500 text-center py-4">No activity yet</p>}
            </div>
        ...
        <div className="space-y-6">
          ...
          <div className="card">
            ...
            <dl className="space-y-2 text-sm">
              <div className="flex justify-between"><dt className="text-gray-500">Analyst</dt><dd className="font-medium">{c.analyst_first_name ? `${c.analyst_first_name} ${c.analyst_last_name}` : 'Unassigned'}</dd></div>
              <div className="flex justify-between"><dt className="text-gray-500">Type</dt><dd className="capitalize">{c.case_type?.replace('_', ' ')}</dd></div>
              <div className="flex justify-between"><dt className="text-gray-500">Created</dt><dd>{formatDate(c.created_at)}</dd></div>
              <div className="flex justify-between"><dt className="text-gray-500">Updated</dt><dd>{formatRelativeTime(c.updated_at)}</dd></div>
              {c.resolved_at && <div className="flex justify-between"><dt className="text-gray-500">Resolved</dt><dd>{formatDate(c.resolved_at)}</dd></div>}
            </dl>
          </div>
          ...
          {c.tags?.length > 0 && (
            <div className="card">
              ...
              <div className="flex flex-wrap gap-2">
                {c.tags.map((tag: string) => <span key={tag} className="px-2 py-1 bg-gray-100 text-gray-700 rounded text-xs">{tag}</span>)}
              </div>
            </div>
          )}
        </div>
