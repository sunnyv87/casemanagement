  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <h1 className="text-2xl font-bold text-gray-900">SIEM/SOAR Integrations</h1>
          ...
          {data?.data?.map((integration: any) => (
            <div key={integration.id} className="card">
              ...
              <dl className="space-y-2 text-sm mb-4">
                <div className="flex justify-between"><dt className="text-gray-500">Customer</dt><dd className="font-medium">{integration.customer_name}</dd></div>
                <div className="flex justify-between"><dt className="text-gray-500">Mode</dt><dd className="capitalize">{integration.ingestion_mode}</dd></div>
                <div className="flex justify-between"><dt className="text-gray-500">Polling</dt><dd>{integration.polling_interval_seconds}s</dd></div>
                <div className="flex justify-between"><dt className="text-gray-500">Last Poll</dt><dd>{integration.last_poll_at ? formatRelativeTime(integration.last_poll_at) : 'Never'}</dd></div>
                {integration.bidirectional_sync && <div className="flex justify-between"><dt className="text-gray-500">Bidirectional</dt><dd className="text-green-600">Yes</dd></div>}
              </dl>
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-lg max-h-[90vh] overflow-y-auto p-6">
            ...
            <form onSubmit={e => { e.preventDefault(); createMutation.mutate(newIntegration); }} className="space-y-4">
              ...
              <div><label className="block text-sm font-medium text-gray-700 mb-1">Name *</label><input type="text" className="input-field" required value={newIntegration.name} onChange={e => setNewIntegration(i => ({ ...i, name: e.target.value }))} placeholder="e.g., Acme Securonix Production" /></div>
              <div><label className="block text-sm font-medium text-gray-700 mb-1">Base URL *</label><input type="url" className="input-field" required value={newIntegration.base_url} onChange={e => setNewIntegration(i => ({ ...i, base_url: e.target.value }))} placeholder="https://instance.example.com" /></div>
              <div>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div><label className="block text-sm font-medium text-gray-700 mb-1">Polling Interval (s)</label><input type="number" className="input-field" min={30} value={newIntegration.polling_interval_seconds} onChange={e => setNewIntegration(i => ({ ...i, polling_interval_seconds: parseInt(e.target.value) }))} /></div>
